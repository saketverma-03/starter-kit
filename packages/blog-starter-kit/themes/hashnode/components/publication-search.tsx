...
const PublicationSearch = (props: Props) => {
	...
	const onKeywordChange = () => {
		if (
			...
			searchField &&
			searchField.current &&
			searchField.current.value.trim().length === 0
		) {
				...
				>
					...
					<div className="relative w-full">
						{isInputEmpty && (
							<div className="my-4 flex flex-row items-center justify-center text-slate-500 dark:text-slate-300">
								...
								<DialogPrimitive.Title className="font-normal">
					...
					<CustomScrollArea>
						<div className="mt-2 rounded-xl bg-white pb-10 dark:bg-slate-900 md:mt-5">
										...
										>
											...
											>
												<div className="md:mr-4">
													<h1 className="hn-break-words mb-2 break-words text-2xl font-bold leading-snug tracking-tight text-slate-900 dark:text-slate-300">
														{post.title}
													</h1>
							...
							{!fetching && !isResultEmpty && pageInfo?.hasNextPage && (
								<Waypoint onEnter={fetchMore} topOffset="-5%" />
							)}
							{isEndOfFeed && (
								<div className="font-heading self-center py-6 text-center font-semibold text-slate-700 dark:text-slate-300">
